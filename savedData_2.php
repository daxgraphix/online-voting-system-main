...
$sqlCheck = "SELECT * FROM users_2nd WHERE email = :email OR number = :number OR rollno = :rollno";
...
if ($existingVote) {
    ...
    echo "<script>alert('You have already voted.'); window.location.href = 'validation_2_user.php';</script>";
    exit();  // Make sure to stop further script execution
} else {
    // User has not voted, proceed with inserting the vote
    $sql = "INSERT INTO users_2nd(name, email, Branch, number, candidate, rollno, reason) 
